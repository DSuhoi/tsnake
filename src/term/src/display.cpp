...
// Delete the window
void Display::delete_window(WINDOW *win)
{
...
// Print the text
void Display::print_scr(WINDOW *win, int x, int y, char *buff)
{
...
void Display::print_scr(WINDOW *win, int x, int y, char *buff, int color)
{
    wattron(win,COLOR_PAIR(color));   // Setting the color
    print_scr(win, x, y, buff);
    wattroff(win,COLOR_PAIR(color));  // Disable the color
    wrefresh(win);                    // and updating the window
}

// Print the character
void Display::print_scr(WINDOW *win, int x, int y, chtype ch)
{
